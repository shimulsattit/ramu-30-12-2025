...
@section('content')
    ...
    <div class="container mb-4">
        ...
        @if($displayMessages->count() > 0)
            <div class="row g-3">
                @foreach($displayMessages as $item)
                    @if($messageCardDesign === 'image')
                        @include('partials.message-cards.image', ['message' => $item['message'], 'title' => $item['title']])
                    @elseif($messageCardDesign === 'modern')
                        @include('partials.message-cards.modern', ['message' => $item['message'], 'title' => $item['title']])
                    @else
                        @include('partials.message-cards.solid', ['message' => $item['message'], 'title' => $item['title']])
                    @endif
                @endforeach
            </div>
        @endif

            ...
            <div class="row mb-0">
                <div class="col-12">
                    <marquee onmouseover="this.stop();" onmouseout="this.start();"
                        style="font-weight: 500; font-size: 0.95rem; color: #fff; padding: 2px 0; margin: 0;">
                        @php
                        ...
                        @if(isset($tickerNotices) && $tickerNotices->count() > 0)
                            @foreach($tickerNotices as $notice)
                                <a href="{{ $notice->link_url }}" class="text-decoration-none me-5"
                                    style="transition: all 0.3s ease; font-size: 0.95rem; color: #fff;">
                                    <i class="fas fa-bell"
...
<nav class="navbar navbar-expand-lg navbar-custom sticky-top" style="background-color: var(--navbar-bg-color, #1e5540) !important; min-height: 50px; display: block !important; visibility: visible !important; padding: 0 !important; border-bottom: 2px solid rgba(0,0,0,0.1);">
    <div class="container">
        {{-- Force display on all screens, but still include toggler for responsive feel if needed --}}
